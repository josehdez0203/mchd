...
class Usuario
{
    ...
    private function headers()
    {
        ...
        //Cargamos sesion
        // session_set_cookie_params(time() + (60 * 60 * 24 * 2));
        session_start();
    }
    ...
    public function ApiLogin()
    {
        ...
        if (!empty($_POST['email']) or !empty($_POST['pass'])) {
            ...
            $consulta = $this->db->query("SELECT * FROM usuarios 
              WHERE email='$this->email' AND password='$this->pass'");
            if ($this->db->rows($consulta) > 0) {
                ...
                if ($datos['activo'] == 0) {
                    ...
                    $_SESSION['tipo'] = $this->Encript($datos['tipo'] . 'jhc');
